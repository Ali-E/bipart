...
/*
	Desc: UBUBPartitionFunction class computes an upper bound for the partition function. It uses the Energy class to compute energies. This version accomodates a nonsaturated window sublength.
...
#define INITVALUE (std::numeric_limits<double>::infinity())
...
UBPartitionFunction::UBPartitionFunction(UBPartitionFunction *p)
{
	double temp;
	energy = p->getEnergy();
	...
	perturbation[0] = p->getPerturbation(0);
	perturbation[1] = p->getPerturbation(1);
	sublen = p->getSubLen();
...
UBPartitionFunction::UBPartitionFunction(Energy *en, Sequence *seq, Table<double> *pertrb[2], bool _debug, int _procNum, bool _quiet, bool _no_isolate)
{
	double temp;
	energy = en;
	energy->getParams(&NA, &polymer, &naConc, &mgConc, &suffix, &temp, &zip, &nodangle, &energyType);
	...
	parFuncType = SINGLE;
	sequence = seq;
	perturbation[0] = pertrb[0];
	perturbation[1] = pertrb[1];
	sublen = seq->getLen();
	parfunc[0] = parfunc[1] = NULL;
	QI = NULL;
	char d2 = 2;
	...
	if(!quiet) printf("Allocating memory for single estimate of %s... \n", sequence->getName());
	for(int j = 0; j < 3; j++)
	{
		Q[j] = new Table<double>(sequence->getLen(), sublen, &d2, INITVALUE);
		Qm[j] = new Table<double>(sequence->getLen(), sublen, &d2, INITVALUE);
	}
	Qb = new Table<double>(sequence->getLen(), sublen, &d2, INITVALUE  );

	if(!quiet) printf("Allocation and initialization finished. \n");
	computeSingleParFunc();
}

UBPartitionFunction::UBPartitionFunction(FILE *inp, Energy *en, Sequence *seq, Table<double> *pertrb[2], bool _debug, int _procNum, bool _quiet, bool _no_isolate)
{
	double temp;
	energy = en;
	energy->getParams(&NA, &polymer, &naConc, &mgConc, &suffix, &temp, &zip, &nodangle, &energyType);
...
UBPartitionFunction::UBPartitionFunction(Energy *en, Sequence *seq, Table<double> *pertrb[2], unsigned int _sublen, bool _debug, int _procNum, bool _quiet, bool _no_isolate)
{
	double temp;
	energy = en;
	energy->getParams(&NA, &polymer, &naConc, &mgConc, &suffix, &temp, &zip, &nodangle, &energyType);
...
UBPartitionFunction::UBPartitionFunction(Energy *en, Table<double> *pertrb[2], UBPartitionFunction *par[2], bool _debug, int _procNum, bool _quiet, bool _no_isolate)
{
	double temp;
	energy = en;
	energy->getParams(&NA, &polymer, &naConc, &mgConc, &suffix, &temp, &zip, &nodangle, &energyType);
	...
	if(!quiet) printf("Allocating memory for double estimate of %s and %s... \n", parfunc[0]->getSequence()->getName(), parfunc[1]->getSequence()->getName());
	...
	for(int i = 0; i < 3; i++)
	{
		for(int j = 0; j < 3; j++)
		{
			if(i+j) QIa_nn[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			if(i+j) QI_nn[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		}
	}

	for(int i = 0; i < 2; i++)
	{
		QIh[i] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QIx_m[i][2] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QImm[i][2] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QImm[2][i] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QImk[i][1] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QImk[2][i] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QIkm[1][i] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QIkm[i][2] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QIa_nd[i][1] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QIa_nd[2][i] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QI_nd[i][1] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QI_nd[2][i] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QIa_dn[1][i] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QIa_dn[i][2] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QI_dn[1][i] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		QI_dn[i][2] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		for(int j = 0; j < 2; j++)
		{
			QIb[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			QIhh[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			QIhm[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			QIx_m[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			QIx_k[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			if(i+j) QIkk[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			if(i+j) QIa_r[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			if(i+j) QI_r[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			if(i+j) QIlr[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			if(i+j) QIa_dd[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
			if(i+j) QI_dd[i][j] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
		}
	}
	QImm[2][2] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
	QIa_nd[1][0] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
	QI_nd[1][0] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
	QIa_dn[0][1] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
	QI_dn[0][1] = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
	QIy = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
	QI = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
	QIa = new Table<double>(l1, l2, subl1, subl2, INITVALUE);
...
UBPartitionFunction::UBPartitionFunction(FILE *inp, Energy *en, Table<double> *pertrb[2], UBPartitionFunction *par[2], bool _debug, int _procNum, bool _quiet, bool _no_isolate)
{
	double temp;
	energy = en;
	energy->getParams(&NA, &polymer, &naConc, &mgConc, &suffix, &temp, &zip, &nodangle, &energyType);
...
UBPartitionFunction::~UBPartitionFunction()
{
...
double UBPartitionFunction::freeEnsembleEnergy(double h)
{
	return RT*Zz*h;
}

double UBPartitionFunction::parFunc(double eng)
{
	return exp(-eng/RT);
}

void UBPartitionFunction::Reverse()
{
	if(parFuncType == SINGLE)
	{
		for(int j = 0; j < 3; j++)
...
double UBPartitionFunction::computeSingleParFunc()
{
	if(parFuncType == DUPLEX)
		return 0.0;

	if(!quiet) printf("Computing single estimate for %s whose length is %d with sublength %d... \n", sequence->getName(), sequence->getLen(), sublen);
...
#pragma omp parallel for num_threads(procNum)
	...
	if(!quiet) printf("Computing single estimate finished. \n");
...
void UBPartitionFunction::computeSingleQbQm(int i, int j)
{
	...
	if(j > i+3 && energy->base_pair(seq[i], seq[j]))
	{
			...
			for(int e = d+4; e <= j-1; e++)
			{
				...
				if (e == j-1 && d == i+1)
				{
					//*b += Qb->element(d, e)*exp(-energy->Es(i, j, seq)/RT);
					 localscore = Qb->element(d, e)+energy->Es(i, j, seq)/RT;
				}	
				else 
				{
					//*b += Qb->element(d, e)*exp(-energy->Ebi(i, j, d, e, seq)/RT);
					localscore = Qb->element(d, e)+energy->Ebi(i, j, d, e, seq)/RT;
					if(d > i+1)
					{
						//*b += Qb->element(d, e)*Qm[2]->element(i + 1, d - 1)*exp(-energy->Emulti(1, j-e-1, 1)/RT); 
						double localscore2 = Qb->element(d, e)+Qm[2]->element(i + 1, d - 1)+energy->Emulti(1, j-e-1, 1)/RT;
						localscore = MIN(localscore, localscore2);
					}
				}
		...
		*b -= perturbation[0]->element(i, j)-perturbation[1]->element(i, j);
	}
	...
	for(int d = i; d <= j - 4; d++)
		for(int e = d + 4; e <= j; e++)
		{
			...
			*mr = Qb->element(d, e)+energy->Ekissing(0, j-i-(e-d), 1)/RT;

			//*mg += Qb->element(d, e)*exp(-energy->Emulti(0, j-i-(e-d), 1)/RT); 
			*mg = Qb->element(d, e)+energy->Emulti(0, j-i-(e-d), 1)/RT; 
			if (d > i)
			{ 
				double lsw, lsr, lsg;
				//*mw += Qm[0]->element(i, d-1)*Qb->element(d, e); 
				lsw = Qm[0]->element(i, d-1)+Qb->element(d, e);
				*mw = MIN(*mw, lsw);
				
			  	//*mr += Qm[1]->element(i, d-1)*Qb->element(d, e)*exp(-energy->Ekissing(0, j-e, 1)/RT); 
				lsr = Qm[1]->element(i, d-1)+Qb->element(d, e)+energy->Ekissing(0, j-e, 1)/RT;
				*mr = MIN(*mr, lsr);

   				//*mg += Qm[2]->element(i, d-1)*Qb->element(d, e)*exp(-energy->Emulti(0, j-e, 1)/RT);
				lsg = Qm[2]->element(i, d-1)+Qb->element(d, e)+energy->Emulti(0, j-e, 1)/RT;
				*mg = MIN(*mg, lsg);
			}					
...
void UBPartitionFunction::computeSingleQ(int i, int j)
{
	//*(Q[0]->estar(i, j)) = 1.0;
	(*Q[0])(i, j) = 0.0;

	//*(Q[1]->estar(i, j)) = exp(-energy->Ekissing(0, j - i + 1, 0)/RT);
	(*Q[1])(i, j) = energy->Ekissing(0, j - i + 1, 0)/RT;

	//*(Q[2]->estar(i, j)) = exp(-energy->Emulti(0, j - i + 1, 0)/RT);
	(*Q[2])(i, j) = energy->Emulti(0, j - i + 1, 0)/RT;
	...
	for(int d = i; d <= j-4; d++)
		for(int e = d+4; e <= j; e++)
		{
			...
			(*Q[0])(i, j) = MIN((*Q[0])(i, j), temp1+Qb->element(d, e));

   			//*(Q[1]->estar(i, j)) += temp2*(Qb->element(d, e))*exp(-energy->Ekissing(0, j-e, 1)/RT);
			(*Q[1])(i, j) = MIN((*Q[1])(i, j), temp2+Qb->element(d, e)+energy->Ekissing(0, j-e, 1)/RT);
		
   			//*(Q[2]->estar(i, j)) += temp3*(Qb->element(d, e))*exp(-energy->Emulti(0, j-e, 1)/RT);
			(*Q[2])(i, j) = MIN((*Q[2])(i, j), temp3+Qb->element(d, e)+energy->Emulti(0, j-e, 1)/RT);
		}
}
...
double UBPartitionFunction::computeDoubleParFunc()
{
	...
	if(!quiet) printf("Computing double estimate for %s and %s whose lengths are %d and %d with sublengths %d and %d... \n", sequenceUp->getName(), sequenceDown->getName(), sequenceUp->getLen(), sequenceDown->getLen(), sublenUp, sublenDown);
...
/*		for(unsigned int l2 = 1; l2 <= l1; l2++)
	...
	if(!quiet) printf("Computing double estimate finished. \n");
...
double UBPartitionFunction::product(Table<double> *T1, Table<double> *T2, int i1, int j1, int i2, int j2)
{
	double result = 0.0;
...
double UBPartitionFunction::rsemiproduct(Table<double> *Tu, Table<double> *Td, Table<double> *T2, int i1, int j1, int i2, int j2)
{
	double result = 0.0;
...
double UBPartitionFunction::lsemiproduct(Table<double> *T1, Table<double> *Tu, Table<double> *Td, int i1, int j1, int i2, int j2)
{
	double result = 0.0;
	for(int d = i1; d <= j1; d++){
		double temp1 = 1.0;
...
void UBPartitionFunction::computeBands(int i1, int j1, int i2, int j2)
{
...
double UBPartitionFunction::capproduct(Table<double> *qir, Table<double> *qig, Table<double> *qi, int i1, int j1, int i2, int j2, int upOrDown)
{
	double result = 0.0;
...
void UBPartitionFunction::computeXY(int i1, int j1, int i2, int j2)
{
	//QIy
...
void UBPartitionFunction::computeAux(int i1, int j1, int i2, int j2)
{
	if(!energy->base_pair(seqUp[j1], seqDown[j2]))
		return;
...
void UBPartitionFunction::computeImmkk(int i1, int j1, int i2, int j2)
{
	//QImm
...
void UBPartitionFunction::computeIa_nndd(int i1, int j1, int i2, int j2)
{
	//QIa_nn
	for(int i = 0; i < 3; i++)
		for(int j = 0; j < 3; j++)
...
void UBPartitionFunction::computeI_nndd(int i1, int j1, int i2, int j2)
{
	//QI_nn
	for(int i = 0; i < 3; i++)
		for(int j = 0; j < 3; j++)
...
void UBPartitionFunction::computeI(int i1, int j1, int i2, int j2)
{
	//QIa
...
void UBPartitionFunction::store(FILE *outp)
{
	if(parFuncType == SINGLE)
	{
		if(!quiet) printf("Storing single estimate of %s... \n", sequence->getName());
		for(int j = 0; j < 3; j++)
		{
			Q[j]->store(outp);
			Qm[j]->store(outp);
	...
	{
		if(!quiet) printf("Storing double estimate of %s and %s... \n", parfunc[0]->getSequence()->getName(), parfunc[1]->getSequence()->getName());
...
void UBPartitionFunction::retrieve(FILE *inp)
{
	if(parFuncType == SINGLE)
	{
		if(!quiet) printf("Retrieving single estimate of %s... \n", sequence->getName());
		for(int j = 0; j < 3; j++)
		{
			Q[j]->retrieve(inp);
			Qm[j]->retrieve(inp);
	...
	{
		if(!quiet) printf("Retrieving double estimate of %s and %s... \n", parfunc[0]->getSequence()->getName(), parfunc[1]->getSequence()->getName());
...
void UBPartitionFunction::selftest(int i1, int j1, int i2, int j2, int l1, int l2)
{
